...
var get_next=document.querySelector(".btn-next");
get_next.addEventListener("click",(eve)=>{

   eve.preventDefault()
  console.log("clicked");
  var user_data=data.length;
  var inter=(parseInt(user_data/5))*5;
  for(var i = 1;i<6;i++){
  var get_btn=document.querySelector(`.btn-${i}`);
  var get_for=parseInt(get_btn.getAttribute("for"));
  
  if(inter>=get_for){
    var inc_for=get_for+5;
    get_btn.setAttribute("for",inc_for)
    get_btn.innerText=inc_for;
    }else{
      get_btn.remove()
    }
...
var get_previous=document.querySelector(".btn-previous");
get_previous.addEventListener("click",()=>{
